...
import { credentialedCors } from './lib/http';
...
const authedCors = credentialedCors(['Content-Type', 'Authorization'], ['GET', 'POST', 'PATCH', 'DELETE', 'OPTIONS']);
...
app.use('/auth/*', (c, next) =>
  credentialedCors(['Content-Type', 'Authorization'], ['GET', 'POST', 'OPTIONS'])(c, next),
);
// 公共端点：admin UI 用它判断是否该显示 /signup
// 与 /auth/* 不同名，所以要单独挂 CORS（不然浏览器 fetch 会被拦）
app.use('/auth-meta', credentialedCors(['Content-Type'], ['GET', 'OPTIONS']));
